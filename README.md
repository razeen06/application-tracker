...
- **Gmail-based status suggestions** — an optional read-only Gmail connection scans for messages matching tracked applications and classifies each one (Interview Offered / Action Required / Progress / Rejected / Not Relevant) via Gemini. Nothing is applied automatically; a suggestion sits as a banner on the dashboard until accepted or overridden by a manual status change.
...
- **Application Funnel** — a Sankey diagram of how tracked applications actually resolve: Interview, Offer, Rejected, Action Required, Progress, or a derived **Ghosted** bucket for applications still sitting at "Applied" past a configurable day threshold. Ghosted is computed fresh on every request, never stored, since a company can always still respond after the threshold passes. Collapses to a compact summary bar by default; click to expand into the full diagram.
